...
# Known error image hashes (MD5) - tiles matching these will abort
# the download
ERROR_IMAGE_HASHES = {
...
class DownloadSession:
    ...
    def is_cancelled(self):
        ...
        # Set = cancelled (standard semantics)
...
def check_for_error_image(file_path, session_obj):
    ...
    try:
        ...
        if file_hash in ERROR_IMAGE_HASHES:
            error_msg = (
                f"Error image detected (hash: {file_hash}). "
                f"Map provider returned an error image instead of "
                f"valid tile data."
            )
...
def download_tile(
    ...
    try:
        if cache_check_path.exists():
            # If we need 8bit version but don't have it yet,
            # convert from normal
            if convert_to_8bit and cache_check_path == normal_tile_path:
                try:
                    ...
                    logging.debug(
                        f"Cache file disappeared during copy: "
                        f"{cache_check_path}"
                    )
                    # Fall through to download
                except Exception as e:
                    logging.warning(
                        f"Cache conversion failed: {e}, will re-download"
                    )
                    # Fall through to download
                else:
        ...
        logging.debug(
            f"Cache check failed: {e}, proceeding to download"
        )
        # Fall through to download
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if not content_type.startswith('image/'):
                    logging.warning(
                        f"Invalid content type '{content_type}' for "
                        f"tile {tile} (expected image/*)"
                    )
                    ...
                    try:
                        ...
                        if check_for_error_image(
                            ...
                            try:
                                ...
                                logging.warning(
                                    f"Failed to remove error image "
                                    f"{normal_tile_path}: {e}"
                                )
                ...
                # Removed socket emission for downloaded tiles
                ...
                if (session_obj.rate_limit_counter >=
                        RATE_LIMIT_DETECTION_THRESHOLD):
                    session_obj.currently_throttled = True
                    logging.warning(
                        f"Rate limit detected for session "
                        f"{session_obj.session_id}, throttling downloads"
                    )
            ...
            if attempt < max_retries - 1:
                ...
                logging.error(
                    f"Failed to download tile "
                    f"{tile.z}/{tile.x}/{tile.y}: {e}"
                )
...
def get_tiles_for_polygons(polygons_data, min_zoom, max_zoom):
    ...
    polygons = [
        Polygon([(lng, lat) for lat, lng in poly])
...
def download_tiles_optimized(
    ...
    try:
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            ...
            for tile in tiles:
                if session_obj.is_cancelled():
                    ...
                    for future in all_futures:
                        future.cancel()
                    logging.info(
                        f"Cancelled {len(all_futures)} pending futures"
                    )
                    break
            ...
            for future in as_completed(futures):
                ...
                if result['status'] == 'error_image':
                    logging.error(
                        f"Error image detected at tile "
                        f"{result['tile'].z}/{result['tile'].x}/"
                        f"{result['tile'].y}"
                    )
                ...
                if result['status'] == 'failed' and len(retry_queue) < 1000:
        ...
        logging.debug("ThreadPoolExecutor cleanup complete")
    ...
    try:
        ...
        for i, poly in enumerate(polygons_data):
            if not isinstance(poly, list) or len(poly) < 3:
                emit(
                    'error',
                    {'message': f'Polygon {i+1} must have at least 3 points'}
                )
                return
        ...
        # Validate map style
        style_name = next(
            (name for name, url in MAP_SOURCES.items()
        ...
        normal_cache_dir = (
            get_style_cache_dir(style_name, False)
            if convert_to_8bit
            else None
        )
        ...
        # Clear event (not cancelled)
        session_obj.cancel_event.clear()
        session_obj.stats = {
            'total': 0,
            'completed': 0,
            'skipped': 0,
        ...
        def download_task():
            try:
                ...
                if not session_obj.is_cancelled():
                    if create_zip_file:
                        ...
                        with app.app_context():
                            socketio.emit(
                                'download_complete',
                                {'zip_url': f'/download_zip?path={zip_path}'},
                                to=session_id
                            )
                    else:
                        with app.app_context():
                            socketio.emit(
                                ...
                                {
                                    'zip_url': None,
                                    'message': 'Tiles cached successfully'
                                },
                                to=session_id
                            )
                ...
                logging.error(
                    f"Download task failed for session {session_id}: {e}",
                    exc_info=True
                )
                try:
                    with app.app_context():
                    ...
                    logging.error(
                        f"Failed to emit error to client: {emit_err}"
                    )
            finally:
...
def handle_cancel_download():
    ...
    session_id = request.sid
    session_obj = get_session(session_id)
    # User-initiated cancellation has no error message
    session_obj.cancel()
...
def serve_tile(style_name, z, x, y):
    ...
    Checks both 8-bit and normal directories.
    """
    ...
    if not (0 <= x < max_coord and 0 <= y < max_coord):
        return (
            f'Invalid tile coordinates for zoom {z} '
            f'(must be 0-{max_coord-1})',
            400,
        )
...
def delete_cache(style_name):
    """Delete cache directory for a style.

    Deletes both 8-bit and normal cache directories.
    """
    sanitized_name = sanitize_style_name(style_name)
...
def get_cached_tiles_route(style_name):
    """Return list of [z, x, y] for cached tiles.

    Supports optional filtering by zoom level and coordinates.
    """
    ...
    # Current + N deeper levels
    cache_zoom_range = request.args.get(
        'cache_zoom_range',
        default=1,
        type=int,
    )

    # Use set to avoid duplicates from 8-bit and normal caches
    cached_tiles = set()
        ...
        for z_dir in style_cache_dir.iterdir():
            if z_dir.is_dir():
                try:
                    ...
                    # cache_zoom_range parameter: show current zoom +
                    # cache_zoom_range deeper levels
                    # For cache_zoom_range=2: shows zoom, zoom+1, zoom+2
                    # (3 levels total)
                    # For cache_zoom_range=0: shows only current zoom
                    # (1 level)
                    if zoom is not None and (
                        z < zoom or z > zoom + cache_zoom_range
                    ):
                        continue
                    ...
                    for x_dir in z_dir.iterdir():
                        if x_dir.is_dir():
                            try:
                                ...
                                # (ONLY for current zoom level)
                                # For other zoom levels, frontend will
                                # filter by viewport
                                if (z == zoom and min_x is not None
                                        and x < min_x):
                                    continue
                                if (z == zoom and max_x is not None
                                        and x > max_x):
                                    continue
                                ...
                                for y_file in x_dir.glob('*.png'):
                                    try:
                                        ...
                                        # Filter by y coordinate if
                                        # specified (ONLY for current zoom)
                                        if (z == zoom and min_y is not None
                                                and y < min_y):
                                            continue
                                        if (z == zoom and max_y is not None
                                                and y > max_y):
                                            continue
...
def estimate_tile_count():
    """Calculate exact tile count for given polygons and zoom range."""
